...
def run_scheduler(message, os_handler):
    ...
    if time.time() > target_timestamp:
        print(
            f"Error: The target time ({target_time.strftime('%H:%M:%S.%f')[:-3]}) has already passed today!"
        )
        return
    ...
    try:
        ...
        while True:
            ...
            print(
                f"Time remaining: {remaining:.1f}s | Status: Waiting... Clipboard is free! ",
                end="\r",
                flush=True,
            )
            time.sleep(0.1)
        ...
        while True:
            ...
            remaining = target_timestamp - current_time
            print(
                f"Time remaining: {remaining:.1f}s | Status: Message pasted! Readying Enter...                 ",
                end="\r",
                flush=True,
            )
            time.sleep(0.05)
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] TARGET HIT! Enter keystroke sent!"
        )
...
if __name__ == "__main__":
    print("--- Tibialy Discord Precision Scheduler ---\n")
